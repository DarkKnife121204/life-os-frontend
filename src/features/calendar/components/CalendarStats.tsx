            ...
            <div className="grid grid-cols-2 gap-3">
                {calendarStats.map((stat) => {
                    ...
                    return (
                        <div
                            key={stat.label}
                            className="min-h-[74px] rounded-lg border border-cyan-400/10 bg-[#06111a]/70 px-4 py-3 shadow-[inset_0_0_20px_rgba(0,255,255,0.03)]"
                        >
                            <div className="flex items-start gap-3">
                                {Icon && <Icon className={`h-6 w-6 shrink-0 ${color}`} />}

                                <div>
                                    <div className="text-base leading-none text-zinc-100">{stat.value}</div>

                                    <div className="mt-2 text-xs leading-none text-zinc-300">{stat.label}</div>
                                </div>
                            </div>
                        </div>
                    );
