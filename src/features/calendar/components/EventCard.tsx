    ...
    return (
        ...
        >
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2 text-zinc-100">
                        <div className="flex min-w-0 items-center gap-1">
                            <span
                                className={`h-2.5 w-2.5 shrink-0 rounded-full ${DOT_PRIORITY_CLASSES[event.priority]}`}
                            />
                    ...
                    {timeView === "block" && <div className="mt-1 text-zinc-300">{timeText}</div>}

                    {showDescription && event.description && (
                        <p className="mt-1 line-clamp-2 text-xs text-zinc-400">{event.description}</p>
                    )}
                </div>
            </div>
            <div className={`flex items-center justify-center gap-2`}>
                {showType && (
                    <span
                        className={` rounded-md px-2 py-1 text-xs ${BG_COLOR_CLASSES[event.color]} ${TEXT_COLOR_CLASSES[event.color]}`}
                    >
                        {event.type}
                    </span>
                )}
