...
export default function CalendarAllEvents({events, isLoading = false, meta, page, setPage, sortField, sortDirection, onSort,
    ...
    const paginationPages = meta
        ? getPaginationPages(meta.current_page, meta.last_page)
        : [];
            ...
            <div className="flex h-full min-h-0 flex-col p-5">
                <div className="shrink-0 mb-5 grid grid-cols-1 gap-3 lg:grid-cols-[1fr_165px_120px_150px_auto_auto]">
                    <label className="relative block">
                        <input type="text" placeholder="Search events..." className="h-10 w-full rounded-lg border border-cyan-400/15 bg-[#030D14] px-4 pr-12 text-sm text-zinc-100
                            placeholder:text-zinc-400 outline-none transition focus:border-cyan-400/50 focus:shadow-[0_0_18px_rgba(0,255,255,0.15)]"/>
                        <button type="button" className="absolute right-3 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-md text-zinc-500 transition
                        ...
                        <table className="w-full min-w-[980px] border-collapse text-sm">
                            ...
                            <tr className="border-b border-cyan-400/10 bg-cyan-400/5 text-left text-xs text-zinc-300">
                                {TABLE_HEADERS.map((header) => (
                                    <CalendarTableSortHeader key={header.field} label={header.label} field={header.field} activeField={sortField}
                                                             direction={sortDirection} onSort={onSort}/>
                                ))}
                            </tr>
                            </thead>
                            <tbody>
                            {events.map((event) => (
                                <tr key={event.id}
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="mt-1 text-xs text-zinc-400">{event.description}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3">
                                        <div
                                            className={`flex items-center gap-2 ${TEXT_PRIORITY_CLASSES[event.priority]}`}>{event.priority}</div>
                                    </td>
                                    <td className="px-4 py-3"><span
                                        className={`rounded-md px-3 py-1 text-xs ${STATUS_CLASSES[event.status]}`}>{event.status}</span>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
                    ...
                    {meta && (
                        <div className="flex items-center gap-2">
                            ...
                            {paginationPages.map((pageNumber) => {
                                ...
                                return (
                                    <button key={pageNumber} type="button" disabled={isLoading} onClick={() => setPage(pageNumber)}
                            ...
                            <button type="button" disabled={page >= meta.last_page || isLoading} onClick={() => setPage((prev) => prev + 1)}
                                className="h-8 w-8 flex items-center justify-center rounded-lg border border-cyan-400/10 text-zinc-500
                                    hover:bg-cyan-400/10 hover:text-cyan-300 transition hover:border-cyan-400/50 hover:shadow-[0_0_16px_rgba(0,255,255,0.14)]
                                    disabled:cursor-not-allowed disabled:opacity-40 cursor-pointer">
