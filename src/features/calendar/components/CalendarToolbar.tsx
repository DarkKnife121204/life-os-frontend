...
export default function CalendarToolbar({
    ...
    const handlePrev = () => {
        setSelectedDate((prevDate) => getPrevDateByView(view, prevDate));
    };
    const handleNext = () => {
        setSelectedDate((prevDate) => getNextDateByView(view, prevDate));
    };
    ...
    return (
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
                <button
                    onClick={handleToday}
                    className="h-10 rounded-lg border border-cyan-400/15 bg-[#030D14] px-4 text-sm text-zinc-300 transition
                ...
                <div className="flex overflow-hidden rounded-lg border border-cyan-400/15 bg-[#030D14] transition hover:border-cyan-400/50 hover:shadow-[0_0_16px_rgba(0,255,255,0.14)]">
                    <button
                        onClick={handlePrev}
                        className="flex items-center justify-center h-10 w-10 text-zinc-300 transition hover:bg-cyan-400/10 hover:text-cyan-300 cursor-pointer "
                    >
            ...
            <div className="flex flex-wrap items-center gap-3">
                <div className="flex overflow-hidden rounded-lg border border-cyan-400/15 bg-[#030D14] transition hover:border-cyan-400/50 hover:shadow-[0_0_16px_rgba(0,255,255,0.14)]">
                    {CALENDAR_VIEWS.map((item) => (
                        <button
                            ...
                            className={`h-10 px-4 text-sm transition cursor-pointer 
                            ${
                                view === item
                ...
                <button
                    onClick={onFilter}
                    className="h-10 cursor-pointer rounded-lg border border-cyan-400/15 bg-[#030D14] px-4 text-sm text-zinc-300 transition
