...
export default function CalendarDay({ selectedDate, events, isLoading = false, onEventClick}: CalendarWeekProps) {
    ...
    const allDayHeight = Math.max(
        ALL_DAY_ROW_HEIGHT,
        allDayEvents.length * EVENT_ROW_HEIGHT + 50
    );
    return (
                ...
                <div className="flex-1 min-h-0 overflow-y-auto overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                    <div className="grid grid-cols-[90px_1fr] min-w-[760px]">
                        <div className="border-r border-cyan-400/10">
                            <div style={{height: `${allDayHeight}px`}}
                                 className="border-b border-cyan-400/10 px-4 py-4 text-sm text-zinc-400">
                                All-day
                            </div>

                            {CALENDAR_WEEK_DAYS.slice(1).map((hour) => (
                                <div key={hour} className="h-[52px] border-b border-cyan-400/10 px-4 py-3 text-sm text-zinc-400">
                                    {hour}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="relative min-h-[1300px]">
                            <div style={{height: `${allDayHeight}px`}} className="border-b border-cyan-400/10 px-2 py-2">
                                <div className="flex flex-col gap-2">
                                    {allDayEvents.map((event) => (
                                        <EventCard key={event.id} event={event} compact showDescription showType showStatus onClick={onEventClick}/>
                                    ))}
                                </div>
                            </div>
                            ...
                            {timedEvents.map((event) => {
                                ...
                                return (
                                    <div key={event.id} style={position} className="absolute left-2 right-2 [&>div]:h-full [&>div]:overflow-hidden">
                                        <EventCard event={event} showDescription showType showStatus timeView={"inline"} onClick={onEventClick}/>
                                    </div>
                                );
                            })}
                        </div>
