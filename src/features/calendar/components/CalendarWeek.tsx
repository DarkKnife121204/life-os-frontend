...
export default function CalendarWeek({ selectedDate, events, isLoading = false, onEventClick }: CalendarWeekProps) {
    ...
    const allDayHeight = Math.max(ALL_DAY_ROW_HEIGHT, maxAllDayEventsCount * EVENT_ROW_HEIGHT + 50);
    return (
                    ...
                    <div className="grid grid-cols-[90px_repeat(7,minmax(160px,1fr))] min-w-[1190px]">
                        <div className="border-r border-cyan-400/10">
                            <div
                                ...
                                className="border-b border-cyan-400/10 px-3 py-3 text-sm text-zinc-400"
                            >
                                All-day
                            </div>
                            ...
                            {CALENDAR_WEEK_DAYS.slice(1).map((hour) => (
                                <div
                                    key={hour}
                                    className="h-[52px] border-b border-cyan-400/10 px-3 py-3 text-sm text-zinc-400"
                                >
                                    {hour}
                                </div>
                            ))}
