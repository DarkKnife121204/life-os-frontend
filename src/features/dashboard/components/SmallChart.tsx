...
export default function SmallChart({ title, value, percent, color, negative = false }: SmallChartProps) {
    const stroke = color === "cyan" ? "#00dfff" : color === "orange" ? "#f97316" : "#ec4899";

    return (
        <Card className="min-h-32 p-4 md:p-5 xl:p-6">
            <h2 className="mb-4 text-lg md:text-xl leading-tight">{title}</h2>
            ...
            <div className="flex items-end justify-between gap-4">
                ...
                <svg
                    className="h-12 w-full max-w-[220px] md:max-w-[260px]"
                    viewBox="0 0 260 60"
