...
export default function TodayTasks() {
    return (
        <Card className="min-h-[260px] p-4 md:p-5 xl:p-6">
            <h2 className="mb-5 text-xl leading-tight">Today Tasks</h2>

            <div className="space-y-1">
                {todayTasks.map((task) => (
                    <div
                        key={task.title}
                        className="flex min-h-10 items-center justify-between gap-3 border-b border-cyan-400/5 py-2"
                    >
                        <div className="flex min-w-0 items-center gap-2">
                            ...
                            <span className="truncate text-sm md:text-base text-zinc-200">{task.title}</span>
                        </div>

                        <span className="shrink-0 text-xs md:text-base text-zinc-400">{task.time}</span>
                    </div>
                ))}
            </div>
