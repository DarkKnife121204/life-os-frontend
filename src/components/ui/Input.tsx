    ...
    if (icon) {
        return (
            <div className={`flex items-center gap-4 rounded-xl border px-3 py-3 transition ${stateClasses}`}>
                ...
                <input{...props} className={`w-full bg-transparent text-white outline-none transition placeholder:text-zinc-500 focus:text-cyan-300 hover:text-cyan-300 ${className}`}/>
            </div>
        );
    }

    return (
        <input {...props} className={`w-full rounded-xl border px-3 py-3 text-white outline-none transition placeholder:text-zinc-500 focus:text-cyan-300 hover:text-cyan-300
