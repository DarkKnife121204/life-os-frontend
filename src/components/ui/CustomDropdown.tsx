    ...
    return (
        <div className="relative w-full sm:w-auto">
            <button type="button" onClick={() => setIsOpen((prev) => !prev)}
                ...
                <PrevIcon className={`pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 h-6 w-6 transition-transform duration-300 ${isOpen ? "rotate-90" : "-rotate-90"}`}/>
            </button>

            {isOpen && (
                <div className="absolute left-0 top-14 z-[9999] w-full rounded-xl border border-cyan-400/30 bg-[#06121a]/95
